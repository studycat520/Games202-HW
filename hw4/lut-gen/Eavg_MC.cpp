...
Vec3f IntegrateEmu(Vec3f V, float roughness, float NdotV, Vec3f Ei) {
    ...
    // 注意：这里的 loop 实际上是多余的，因为我们已经有了 Ei (即 E(mu))。
    // 我们只需要返回被积函数的值。为了配合代码结构，我们在循环里累加然后求平均，
    // 或者直接在循环里加上我们要算的值。
    
    for (int i = 0; i < sample_count; i++) {
        // --- 填写开始 ---
        // 我们要计算 Integral(E(mu) * 2 * mu * dmu)
        // main函数里做了 * step (dmu) 和累加。
        // 这里只需要返回 2 * Ei * mu。
        // 这里的 sample_count 循环其实不需要，但为了兼容框架：
