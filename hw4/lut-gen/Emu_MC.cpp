...
Vec3f IntegrateBRDF(Vec3f V, float roughness, float NdotV) {
    ...
    for (int i = 0; i < sample_count; i++) {
        ...
        if (dot(N, L) > 0.0) {
            ...
            float F = 1.0; // 计算几何遮蔽，F设为1

            // Cook-Torrance BRDF 分子分母
            // fr = (D * G * F) / (4 * NdotV * NdotL)
            // estimator = fr * NdotL / pdf
            // pdf = NdotL / PI
            // Result = (D * G * F) / (4 * NdotV * NdotL) * NdotL / (NdotL / PI)
            //        = (D * G * F * PI) / (4 * NdotV)
            
            float weight = (D * G * F * PI) / (4.0 * NdotV);
